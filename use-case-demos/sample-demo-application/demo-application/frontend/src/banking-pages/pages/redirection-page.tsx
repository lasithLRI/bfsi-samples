...
const RedirectionPage = ({appConfig}:RedirectionPageProps)=>{
    ...
    if(isCancelled){
    ...
    } else if (accountsToAdd.current?.data?.length > 0) {
        const extractFirstAccount = () => {
            if (accountsToAdd.current.type === "single") {
                return accountsToAdd.current.data[0];
            }

            if (accountsToAdd.current.type === "multiple") {
                const firstChunk = accountsToAdd.current.data[0];
                if (Array.isArray(firstChunk)) {
                    const flatList = firstChunk.flatMap((entry: any) => entry.accounts || []);
                    return flatList[0];
                }
                if (firstChunk?.accounts) {
                    return firstChunk.accounts[0];
                }
            }
        ...
        const selectedAccount = extractFirstAccount();
        if (selectedAccount) {
            data = {
