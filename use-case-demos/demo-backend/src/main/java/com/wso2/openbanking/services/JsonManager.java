...
public class JsonManager {
    ...
    public JsonManager() throws Exception {
        try (InputStream stream = JsonManager.class.getResourceAsStream("/obsigning.key")) {
            if (stream == null) {
                throw new IllegalArgumentException("obsigning.key not found in classpath");
            }
    ...
    public String createPS256JWT(String iss,
        ...
        if (customPayload != null) {
            customPayload.forEach(payloadNode::putPOJO);
        }
        ...
        String encodedHeader = base64UrlEncode(mapper.writeValueAsString(headerNode).getBytes());
        String encodedPayload = base64UrlEncode(mapper.writeValueAsString(payloadNode).getBytes());
        String signingInput = encodedHeader + "." + encodedPayload;

        return signingInput + "." + signPS256(signingInput);
    }
}
