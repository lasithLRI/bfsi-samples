      ...
      if (!fromDate) {
          query = {
            ...
            must: [
                ...
                range: {
                    date: {
                    lte: toDate ? toDate : new Date().toISOString().split('T')[0]
                    }
                }
                }
            ]
 ...
 */
public async getMessageChartData(client: OpenSearchClient, timeframe: 'daily' | 'weekly' | 'monthly', direction?: 'inward' | 'outward') {
 ...
 */
public async getAllMessages(client: OpenSearchClient, direction?: string): Promise<MessageDocument[]> {
  const now = Date.now();
      ...
      ? {
          bool: {
            must: [
              {
                term: {
                  "direction.keyword": direction.charAt(0).toUpperCase() + direction.slice(1).toLowerCase()
                }
              }
            ]
          }
 ...
 */
public async getDailyMessages(client: OpenSearchClient, direction?: string): Promise<MessageDocument[]> {
 ...
 */
public async getWeeklyMessages(client: OpenSearchClient, direction?: string): Promise<MessageDocument[]> {
   ...
   */
  public async getMonthlyMessages(client: OpenSearchClient, direction?: string): Promise<MessageDocument[]> {
   ...
   * @param direction optional - 'inward', 'outward', or undefined for both
   */
  public async getDailyChartData(client: OpenSearchClient, direction?: 'inward' | 'outward') {
   ...
   * @param direction optional - 'inward', 'outward', or undefined for both
   */
  public async getWeeklyChartData(client: OpenSearchClient, direction?: 'inward' | 'outward') {
   ...
   * @param direction optional - 'inward', 'outward', or undefined for both
   */
  public async getMonthlyChartData(client: OpenSearchClient, direction?: 'inward' | 'outward') {
 ...
 */
public async getRecentMessages(client: OpenSearchClient, limit: number = 5, direction: string = 'All', period: string = 'All'): Promise<{
    id: string;
    time: string;
...
public async getErrorStatistics(
  ...
  try {
    ...
    const notSupportedErrors = failedMessages.filter(msg => msg.notSupportedError && msg.notSupportedError.trim() !== '').length;
    const invalidErrors = failedMessages.filter(msg => msg.invalidError && msg.invalidError.trim() !== '').length;
    const otherErrors = failedMessages.filter(msg => 
